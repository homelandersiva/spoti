...
function login(req, res) {
  try {
    ...
    if (!process.env.SPOTIFY_CLIENT_ID || !process.env.SPOTIFY_REDIRECT_URI) {
      console.error('❌ Missing Spotify environment variables in auth controller');
    ...
    console.log('📍 Login called. CLIENT_ID:', process.env.SPOTIFY_CLIENT_ID ? '✓ Set' : '✗ UNDEFINED');
    console.log('📍 Login called. REDIRECT_URI:', process.env.SPOTIFY_REDIRECT_URI ? '✓ Set' : '✗ UNDEFINED');
    ...
    res.status(500).json({ error: 'Failed to create Spotify auth redirect.', details: error.message });
  }
}
...
async function callback(req, res) {
  ...
  if (state !== storedState) {
    ...
    return res.status(400).json({ error: 'State mismatch. Please restart the login process.' });
  }
  ...
  try {
    ...
    if (refreshToken) {
      ...
      if (!existingToken) {
        throw new Error('Spotify did not return a refresh token. Ask the user to re-consent.');
      }
    }
...
<html>
  ...
  <body>
    <div class="card">
      ...
      <pre>{\n  "accessToken": "${accessToken}",\n  "refreshToken": "${refreshToken || 'stored previously'}"\n}</pre>
